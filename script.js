// Add the toggleActivities function
...
// Ensure this function is called on page load to set the initial state
document.addEventListener("DOMContentLoaded", () => {
    toggleActivities(); // Optionally, you can call this to start with the list hidden
});
...
document.getElementById("generate-report").addEventListener("click", () => {
    ...
    try {
        ...
        if (recommendationsElement) {
            ...
            const recommendationsList = recommendationsText.split('.').filter(rec => rec.trim() !== '').map(rec => rec.trim()).join('. ');
            addSubHeadingAndText("Recommendations", recommendationsList);
        }
        addPage();
